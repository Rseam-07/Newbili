...
nonisolated enum DynamicJSONValue: Codable, Hashable {
    ...
    private var accountVideoEntry: AccountVideoEntry? {
        ...
        let rawPGCEpisodeID = Self.positiveID(history?["epid"]?.intValue)
            ...
            ?? Self.positiveID(bangumi?["ep_id"]?.intValue)
        ...
        let pgcSeasonID = isPGC
            ? Self.positiveID(object["season_id"]?.intValue)
                ...
                ?? Self.positiveID(bangumi?["season_id"]?.intValue)
                ?? Self.positiveID(bangumiSeason?["season_id"]?.intValue)
                ?? Self.positiveID(bangumiSeason?["id"]?.intValue)
...
nonisolated struct NavUserInfo: Decodable {
    ...
    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        isLogin = container.decodeLossyBoolIfPresent(forKey: .isLogin)
        face = container.decodeLossyStringIfPresent(forKey: .face)
        uname = container.decodeLossyStringIfPresent(forKey: .uname)
        mid = container.decodeLossyIntIfPresent(forKey: .mid)
        wbiImg = try container.decodeIfPresent(WBIImage.self, forKey: .wbiImg)
        levelInfo = try container.decodeIfPresent(NavLevelInfo.self, forKey: .levelInfo)
    }

    var currentLevel: Int? {
        guard let level = levelInfo?.currentLevel, (0...6).contains(level) else { return nil }
        return level
    }
}
...
nonisolated struct NavLevelInfo: Decodable, Hashable, Sendable {
    ...
    enum CodingKeys: String, CodingKey {
        case currentLevel = "current_level"
        case currentMinimumExperience = "current_min"
        case currentExperience = "current_exp"
    ...
    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        currentLevel = container.decodeLossyIntIfPresent(forKey: .currentLevel)
        currentMinimumExperience = container.decodeLossyIntIfPresent(forKey: .currentMinimumExperience)
        currentExperience = container.decodeLossyIntIfPresent(forKey: .currentExperience)
    ...
    var progress: Double? {
        ...
        return min(
            max(
                Double(currentExperience - currentMinimumExperience)
                    / Double(nextLevelExperience - currentMinimumExperience),
                0
            ),
            1
        )
