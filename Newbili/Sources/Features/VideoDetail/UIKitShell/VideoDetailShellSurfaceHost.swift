...
final class VideoDetailShellSurfaceHost: UIView {
    ...
    init(
        ...
        self.surfaceHostView = DirectUIKitPlayerSurfaceHostView(
            ...
            prefersNativePlaybackControls: dependencies.libraryStore.usesNativePlaybackControls
        )
        ...
        overlayHostingController.view.isUserInteractionEnabled = !dependencies.libraryStore.usesNativePlaybackControls
        ...
        hostedSurfaceView.isUserInteractionEnabled = dependencies.libraryStore.usesNativePlaybackControls
        ...
        dependencies.libraryStore.$usesNativePlaybackControls
            .removeDuplicates()
            .sink { [weak self] isEnabled in
                Task { @MainActor [weak self] in
                    guard let self else { return }
                    self.surfaceHostView.setPrefersNativePlaybackControls(isEnabled)
                    self.surfaceHostView.hostedView.isUserInteractionEnabled = isEnabled
                    self.overlayHostingController.view.isUserInteractionEnabled = !isEnabled
                }
            }
            .store(in: &cancellables)
...
private struct SurfaceOnlyPlayerOverlayRoot: View {
    ...
    var body: some View {
        ...
        let usesNativePlaybackControls = libraryStore.usesNativePlaybackControls
        let shouldKeepChromeMounted = keepsChromeMounted && !usesNativePlaybackControls
