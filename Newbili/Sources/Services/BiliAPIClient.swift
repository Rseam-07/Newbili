...
nonisolated enum WatchLaterSortOrder: String, CaseIterable, Identifiable, Sendable {
    case newest
    case oldest
    ...
    var ascendingQueryValue: String {
        self == .oldest ? "true" : "false"
    }
}

nonisolated enum WatchLaterClearScope: Int, CaseIterable, Identifiable, Sendable {
    case invalid = 1
    case viewed = 2
    case all = 0
    ...
    var confirmationMessage: String {
        switch self {
        case .invalid:
            return "确定清空稍后再看中的失效视频吗？"
        case .viewed:
            return "确定清空稍后再看中已看完的视频吗？"
        case .all:
            return "确定清空整个稍后再看列表吗？此操作无法撤销。"
        }
    }
...
nonisolated struct WatchLaterVideoEntryPage {
    let entries: [AccountVideoEntry]
    let totalCount: Int
    let hasMore: Bool
}
...
nonisolated final class BiliAPIClient {
    ...
    func fetchWatchLaterPage(
        page: Int = 1,
        pageSize: Int = 20,
        filter: WatchLaterFilter = .all,
        keyword: String = "",
        sortOrder: WatchLaterSortOrder = .newest
    ) async throws -> WatchLaterVideoEntryPage {
        let snapshot = await requestSnapshot(purpose: .interaction)
        guard snapshot.isLoggedIn else { throw BiliAPIError.missingSESSDATA }
        let normalizedPage = max(1, page)
        let normalizedPageSize = min(max(1, pageSize), 100)
        let signedQuery = try await signedWBIQuery([
            "pn": String(normalizedPage),
            "ps": String(normalizedPageSize),
            "viewed": String(filter.rawValue),
            "key": keyword.trimmingCharacters(in: .whitespacesAndNewlines),
            "asc": sortOrder.ascendingQueryValue,
            "need_split": "true",
        ...
        let totalCount = Self.dynamicObject(response.payload)
            .flatMap { Self.dynamicInt($0["count"]) }
            ?? entries.count
        let loadedCount = ((normalizedPage - 1) * normalizedPageSize) + entries.count
        return WatchLaterVideoEntryPage(
            entries: entries,
            totalCount: max(totalCount, entries.count),
            hasMore: !entries.isEmpty && loadedCount < totalCount
        )
    }

    func addVideoToWatchLater(aid: Int? = nil, bvid: String? = nil) async throws {
        let context = try await requireCSRFContext(for: .interaction)
        let normalizedBVID = bvid?.trimmingCharacters(in: .whitespacesAndNewlines)
        var body = ["csrf": context.csrf]
        if let aid, aid > 0 {
            body["aid"] = String(aid)
        } else if let normalizedBVID, !normalizedBVID.isEmpty {
            body["bvid"] = normalizedBVID
        } else {
            throw BiliAPIError.api(code: -1, message: "缺少有效的 AV 号或 BV 号")
        }
        let response: BiliResponse<EmptyBiliPayload> = try await postForm(
            base: baseURL,
    ...
    func removeVideosFromWatchLater(aids: Set<Int>) async throws {
        let resources = aids.filter { $0 > 0 }.sorted()
        guard !resources.isEmpty else {
            throw BiliAPIError.api(code: -1, message: "没有可移除的视频")
        }
        let context = try await requireCSRFContext(for: .interaction)
        let response: BiliResponse<EmptyBiliPayload> = try await postForm(
            base: baseURL,
            ...
            body: [
                "csrf": context.csrf,
                "resources": resources.map(String.init).joined(separator: ",")
            ],
            cookieHeader: context.snapshot.cookieHeader,
            retryPolicy: .idempotentMutation
        )
    ...
    func clearWatchLater(scope: WatchLaterClearScope) async throws {
        let context = try await requireCSRFContext(for: .interaction)
        var body = ["csrf": context.csrf]
        if let cleanType = scope.cleanType {
            body["clean_type"] = String(cleanType)
        }
        let response: BiliResponse<EmptyBiliPayload> = try await postForm(
            base: baseURL,
            path: "/x/v2/history/toview/clear",
            body: body,
            cookieHeader: context.snapshot.cookieHeader,
            retryPolicy: .idempotentMutation
        )
