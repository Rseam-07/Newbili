...
void main() {
  ...
  test('deletion follows video identity after a refresh reorders rows', () async {
    ...
    expect(controller.loadingState.value.data!.map((item) => item.aid), [3, 1]);
    expect(removedCount, 1);
  });

  test('an already removed video cannot remove a different row or count twice', () async {
  ...
  test('batch removal preserves newly loaded videos and exits selection', () async {
    controller.loadingState.value = Success([
      LaterItemModel(aid: 2),
      LaterItemModel(aid: 3),
