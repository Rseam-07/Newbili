...
CERT_DIR="${CERT_DIR:-}"
PROFILE_PATH="${PROFILE_PATH:-${CERT_DIR:+$CERT_DIR/profile.mobileprovision}}"
PROFILE_PLIST="${TMPDIR:-/tmp}/newbili_distribution_profile.plist"
TIMESTAMP="$(date +%Y%m%d-%H%M%S)"
DERIVED_DATA_PATH="$ROOT_DIR/build/DistributionSignedDerivedData"
ARCHIVE_PATH="$ROOT_DIR/build/archives/newbili-distribution-$TIMESTAMP.xcarchive"
EXPORT_PATH="$ROOT_DIR/build/distribution-ipa/$TIMESTAMP"
EXPORT_OPTIONS_PLIST="${TMPDIR:-/tmp}/newbili_distribution_export_options_$TIMESTAMP.plist"
...
IDENTITY_NAME="${IDENTITY_NAME:-$(security find-identity -v -p codesigning "$KEYCHAIN" 2>/dev/null | sed -n 's/^[[:space:]]*[0-9]*) [[:xdigit:]]\{40\} "\(.*\)"$/\1/p' | grep -F "($TEAM_ID)" | head -1)}"
if [[ -z "$IDENTITY_NAME" ]]; then
  echo "No valid signing identity was found for team $TEAM_ID." >&2
  exit 1
fi
...
if ! xcodebuild archive \
    -project "$ROOT_DIR/Newbili.xcodeproj" \
    -scheme Newbili \
    ...
    PROVISIONING_PROFILE_SPECIFIER="$PROFILE_NAME" > "$LOG_PATH" 2>&1; then
  echo "Archive failed. Log: $LOG_PATH" >&2
  tail -120 "$LOG_PATH" >&2
  exit 1
fi

APP_PATH="$ARCHIVE_PATH/Products/Applications/Newbili.app"
if [[ -d "$APP_PATH" ]]; then
