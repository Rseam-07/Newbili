...
CERT_DIR="${CERT_DIR:-}"
P12_PATH="${P12_PATH:-${CERT_DIR:+$CERT_DIR/certificate.p12}}"
...
if [[ -n "$P12_PATH" ]] && ! security find-identity -v -p codesigning "$KEYCHAIN" | grep -Fq "($TEAM_ID)"; then
  echo "Importing distribution certificate..."
  if [[ -z "${P12_PASSWORD:-}" ]]; then
    read -rs "P12_PASSWORD?P12 password: "
    print
  fi
...
if ! security find-identity -v -p codesigning "$KEYCHAIN" | grep -Fq "($TEAM_ID)"; then
  ensure_wwdr_g3
fi

if ! security find-identity -v -p codesigning "$KEYCHAIN" | grep -Fq "($TEAM_ID)"; then
  echo "A valid signing identity is still unavailable for team: $TEAM_ID" >&2
  echo "Open Keychain Access and confirm the private key is present under the certificate." >&2
  exit 1
fi

IDENTITY_NAME="${IDENTITY_NAME:-$(security find-identity -v -p codesigning "$KEYCHAIN" | sed -n 's/^[[:space:]]*[0-9]*) [[:xdigit:]]\{40\} "\(.*\)"$/\1/p' | grep -F "($TEAM_ID)" | head -1)}"
